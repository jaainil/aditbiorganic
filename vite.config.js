...
function losslessWebpGenerator(options = {}) {
...
const priorityMap = {
  ...
  '/blog': 0.75,
  ...Object.fromEntries(
    productSlugs.map((slug) => [`/products/${slug}`, 0.85]),
  ),
  ...Object.fromEntries(
    serviceSlugs.map((slug) => [`/services/${slug}`, 0.8]),
  ),
  ...Object.fromEntries(blogSlugs.map((slug) => [`/blog/${slug}`, 0.7])),
};
